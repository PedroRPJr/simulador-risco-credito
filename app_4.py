...
def predict_dynamic(model, scaler, base_input, start_inad, selic_trend, months=18):
    """ 
    Simulação Delta Macro:
    O modelo prevê apenas a variação baseada na economia (sem olhar o passado).
    Nós acumulamos essa variação sobre o valor inicial.
    """
    predictions = []
    ...
    for i in range(months):
        # NOTA: Não passamos mais 'target_lag_1' para o input, 
        # pois o modelo foi treinado sem ele!
        ...
        # 3. Prever o DELTA (Variação Pura)
        df_input = pd.DataFrame([current_input])
        df_input = df_input.reindex(columns=feature_names, fill_value=0)
        scaled = scaler.transform(df_input)
        
        raw_pred = model.predict(scaled)
        if isinstance(raw_pred, (np.ndarray, list)): pred_val = float(raw_pred[0])
        else: pred_val = float(raw_pred)
            
        # O pred_val aqui é "Quanto vai variar". Ex: +0.05
        # Acumulamos no valor total
        current_inad += pred_val
        
        # Travas de segurança do valor final
        current_inad = max(0.0, min(25.0, current_inad))
